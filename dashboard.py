...
# --- 3. CSS (FORCED DARK BLUE VISIBILITY) ---
st.markdown("""
    <style>
        ...
        /* Global Text Forced to Dark Blue */
        h1, h2, h3, p, span, label, .stMarkdown, [data-testid="stWidgetLabel"] p {
        ...
        .branding-text { color: #1d428a !important; font-family: 'Oswald', sans-serif; font-weight: 700; font-size: 18px; letter-spacing: 1.5px; }
        ...
        /* Buttons & Inputs */
        div.stButton > button { 
            background-color: #1d428a !important; color: white !important; 
        ...
        .buyer-label { font-size: 12px; font-weight: 700; color: #1d428a; }
        ...
        .pmt-card div { color: white !important; }
    </style>
""", unsafe_allow_html=True)

# --- 4. AUTH PAGE ---
if not st.session_state.authenticated:
    st.markdown('<div style="height: 10vh;"></div><div style="font-family:Inter; font-size:clamp(40px, 10vw, 75px); font-weight:900; color:#1d428a; text-align:center; line-height:0.9; margin-bottom:15px;">UTAH LAND & PROPERTY</div>', unsafe_allow_html=True)
...
# --- 5. LOGIC ---
# Equity Buyer Principal ($310k) = Price ($330k) - $20k Equity
EQ_BUYER_BAL = D["price"] - D["seller_equity"] if D["price"] > 0 else 0

def calc_monthly_pmt(principal, annual_rate, years):
    if annual_rate <= 0 or years <= 0: return principal / (years * 12) if (years*12) > 0 else 0
...
if st.session_state.user_role == "admin":
    ...
    with st.expander("📁 BUYER VETTING & APPROVAL", expanded=True):
        if not D["vault"]: st.info("No documents uploaded.")
            ...
            for idx, doc in enumerate(D["vault"]):
                ...
                v2.markdown(f'<a href="data:application/octet-stream;base64,{b64}" download="{doc["name"]}"><button style="height:30px; font-size:10px;">VIEW</button></a>', unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        a_title = i1.text_area("Title Search", value=D["instr_title"])
        a_escrow = i2.text_area("Escrow Holder", value=D["instr_escrow"])
        a_servicer = i3.text_area("Mortgage Servicer", value=D["instr_servicer"])
...
if show_data and D["address"]:
    st.markdown(f'<h2 style="color:#1d428a;">{D["address"]}</h2>', unsafe_allow_html=True)
else:
    st.markdown('<h2 style="color:#1d428a;">PRIVATE ASSET: UNVETTED</h2>', unsafe_allow_html=True)
...
with col_main:
    ...
    p3.markdown(f'<div class="buyer-card"><div class="buyer-label">Equity Buyer Principal</div><div class="buyer-value">${EQ_BUYER_BAL:,.2f}</div></div>', unsafe_allow_html=True)
    ...
    if not show_data:
        with st.form("gate"):
            ...
            if st.form_submit_button("SUBMIT"):
                if up: D["vault"].append({"name": f"BUYER_{up.name}", "content": up.getvalue()}); st.success("Submitted.")

with col_pmt:
    st.markdown(f'<div class="pmt-card"><div>15-YR TOTAL</div><div style="font-size:32px; font-weight:900;">${total_15:,.2f}</div></div>', unsafe_allow_html=True)
    st.markdown(f'<div class="pmt-card"><div>30-YR TOTAL</div><div style="font-size:32px; font-weight:900;">${total_30:,.2f}</div></div>', unsafe_allow_html=True)
    
    if show_data:
        st.markdown("### SETTLEMENT VAULT")
        for doc in D["vault"]:
            b64 = base64.b64encode(doc['content']).decode()
