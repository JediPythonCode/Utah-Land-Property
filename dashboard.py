...
if not st.session_state.authenticated:
    ...
    <style>
        ...
        /* Ensure the input field box is centered */
        [data-testid="stVerticalBlock"] { align-items: center !important; }
    </style>
    """, unsafe_allow_html=True)

    # Wrap the entire login UI in the centering div
    ...
    # Use a tighter column layout for the login box
...
# ── 3. PROTECTED CONTENT ──────────────────────────────────────────────────
    ...
    # CSS for centering the Dashboard
    st.markdown("""
        <style>
            /* Center all main block elements */
    ...
    if all_files:
        # Columns inside a centered container look best when balanced
    ...
    if images:
        ...
        for idx, img_path in enumerate(images):
            try:
                img_cols[idx % 3].image(img_path, use_container_width=True)
            except:
                continue
    else:
        ...
        if buyer_docs:
            for f_name in buyer_docs:
                with open(f"vault/buyer_docs/{f_name}", "rb") as f_obj:
                    st.download_button(f"📄 Download: {f_name}", f_obj, file_name=f_name, use_container_width=True)
        else:
            st.success("Documents verified. No pending items.")
    else:
        st.subheader("Management: Document Archival")
        # Center the management tools
    ...
    st.markdown(f"<br><small style='color: #999;'>Authenticated as {role}</small>", unsafe_allow_html=True)
import glob
from datetime import datetime
