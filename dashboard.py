...
# --- 2. DATA PERSISTENCE ---
# Logic to prevent the "Login Double-Press" bug: Authenticate BEFORE rendering the UI
...
if not st.session_state.authenticated:
    ...
    st.markdown(f'''
        ...
        /* FIXED ALIGNMENT FOR LOGIN BOX */
        .login-box-wrapper {{ display: flex; justify-content: center; align-items: center; width: 100%; }}
...
if role == "admin":
    with st.expander("🛡️ ADMIN: UTAH LAND & PROPERTY DEAL MANAGEMENT", expanded=True):
        ...
        if m2.button("💾 SAVE ACTIVE SESSION"):
            st.toast("Deal Data Saved Successfully.")
        ...
        # Directly updating session state via number inputs
