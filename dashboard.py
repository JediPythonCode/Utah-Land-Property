...
def initialize_system():
    # Attempt to get secrets, fallback to demo mode if local
    try:
        key = st.secrets.get("secret_key", "L-9_W7_m6m_kE6pX-7m_fX-p_m6m_kE6pX-7m_fX-p=")
        users = dict(st.secrets.get("users", {"admin": {"key": "1234", "role": "Admin"}, "buyer1": {"key": "1234", "role": "Buyer"}}))
...
role, u_id = st.session_state.role, st.session_state.user_id
...
# ── BUYER VIEW (2026 BENTO HUB) ────────────────────────────────────────────────
    ...
    for i, s in enumerate(STAGES):
        ...
        icon = "✅" if active else "○"
        cols[i].markdown(f"<p style='text-align:center; color:{color}; font-size:0.8rem;'>{icon}<br><b>{s}</b></p>", unsafe_allow_html=True)
    ...
    with col_l:
        ...
        st.checkbox("Appraisal: In Progress", value=False)
        st.checkbox("Closing Prep: Verify Wire", value=False)
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('<div class="bento-card">', unsafe_allow_html=True)
        st.subheader("🔐 Secure Vault")
        # Logic to list shared documents for this specific buyer
        st.caption("Documents shared by your loan officer appear here.")
        st.markdown('</div>', unsafe_allow_html=True)

    with col_r:
        st.markdown('<div class="bento-card">', unsafe_allow_html=True)
        st.subheader("📤 SNapp Upload")
        up = st.file_uploader("Drop document to transmit", label_visibility="collapsed")
        if st.button("Transmit to Admin", use_container_width=True):
            if up: st.success("Transmission Complete")
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('<div class="bento-card" style="background:#eff6ff;">', unsafe_allow_html=True)
        st.subheader("🤖 Conversational AI")
        st.text_input("Ask about your loan...", placeholder="What is my current rate?")
        st.markdown('</div>', unsafe_allow_html=True)

    # Market Intelligence Footer
    st.markdown('<div class="bento-card" style="text-align:center;">', unsafe_allow_html=True)
    st.markdown("**2026 Market Intelligence:** Rate lock recommended for current Utah zip codes.")
    st.markdown('</div>', unsafe_allow_html=True)
