...
if "current_deal" not in st.session_state:
    st.session_state.current_deal = {
        ...
        "instr_title": "", "instr_escrow": "", "instr_servicer": "", "disclosures": [""],
        "vault": [], "images": []
    }

D = st.session_state.current_deal

# --- 3. CORRECTED FINANCIAL LOGIC ---
# Equity Buyer Principal ($310k) = Price ($330k) - Seller Equity ($20k)
...
- Assignment Fee (Service): ${D['assignment_fee']:,.2f}
...
st.markdown("""
    <style>
        ...
        .branding-text { color: #1d428a !important; font-family: 'Oswald', sans-serif; font-weight: 700; font-size: 18px; text-transform: uppercase; text-align: center; display: block; margin-bottom: 20px; }
        ...
        label, [data-testid="stWidgetLabel"] p { font-family: 'Oswald', sans-serif !important; color: #1d428a !important; font-weight: 700 !important; text-transform: uppercase !important; font-size: 13px !important; }
        div.stButton > button { background-color: #1d428a !important; color: white !important; border-radius: 4px !important; height: 56px !important; width: 100% !important; font-family: 'Oswald', sans-serif !important; font-weight: 700 !important; text-transform: uppercase !important; }
...
if not st.session_state.authenticated:
    st.markdown('<div style="height: 15vh;"></div><div style="font-family:Inter; font-size:70px; font-weight:900; color:#1d428a; text-align:center; line-height:0.9;">UTAH LAND & PROPERTY</div>', unsafe_allow_html=True)
    st.markdown('<span class="branding-text">Asset protection ● Maximum privacy</span>', unsafe_allow_html=True)
...
if st.session_state.user_role == "admin":
    ...
    with st.expander("📁 BUYER VETTING & APPROVAL", expanded=True):
        if not D["vault"]: st.info("No documents uploaded.")
            ...
            for idx, doc in enumerate(D["vault"]):
                ...
                v2.markdown(f'<a href="data:application/octet-stream;base64,{b64}" download="{doc["name"]}"><button style="width:100%; cursor:pointer;">VIEW</button></a>', unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        a_rate = c3.number_input("Equity Buyer Rate", value=float(D["interest_rate"]))
        a_hoa = c4.number_input("HOA Monthly", value=float(D["hoa_monthly"]))
        
        f1, f2, f3 = st.columns(3)
        a_price = f1.number_input("Price", value=float(D["price"]))
        a_equity = f2.number_input("Seller Equity (Downpayment)", value=float(D["seller_equity"]))
...
if show_data and D["address"]:
    st.markdown(f'<div style="font-family:Inter; font-size:36px; font-weight:900; color:#1d428a;">{D["address"]}</div>', unsafe_allow_html=True)
