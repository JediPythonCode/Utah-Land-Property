...
# --- 3. AUTH TERMINAL (Original Design Preserved, Fully Centered, Double-Click Fix) ---
...
if st.session_state.user_role == "admin":
    with st.expander("🛡️ ADMIN: DEAL MANAGEMENT TERMINAL"):
        ...
        if st.button("UPDATE DEAL", use_container_width=True):
...
if st.session_state.user_role == "admin":
    if st.button("📄 GENERATE PDF SETTLEMENT"):
        ...
        for line in [
            ...
            f"AITD Balance: ${AITD_PRINCIPAL:,.2f}",
...
for doc in D["vault"]:
    ...
    st.markdown(f'<a href="data:application/pdf;base64,{b64}" download="{doc["name"]}">📥 {doc["name"]}</a>', unsafe_allow_html=True)
